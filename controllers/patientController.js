...
// Controller to get all non-scheduled patients
...
//       doctorId: doctorId, // Match appointments for the given doctor
//       appointmentDate: { $gte: startUTC, $lte: endUTC }, // Match appointments within the date range
//     }).populate('patientId'); // Populate the `patientId` field with patient details
...
//     const nonScheduledPatients = await Patient.find({
//       userId: { $nin: patientIds }, // Exclude patients with today's appointments
//     })
//       .populate({
//         path: 'appointments', // Populate the `appointments` virtual field
//         model: 'Appointment', // Use the `Appointment` model for population
//       })
//       .populate({
//         path: 'userId', // Populate the `userId` field
//         model: 'User', // Use the `User` model for population
//         populate: { 
//           path: 'roles', // Further populate the `roles` field within `userId`
//           model: 'Role', // Use the `Role` model for population
//         },
//       });
...
//       return new UserResource(
//         x.userId?.toObject() || {}, // Convert `userId` to an object (fallback to empty object if null)
//         [x], // Include the patient object itself
//         x.appointments || [], // Include the patient's appointments (fallback to an empty array)
